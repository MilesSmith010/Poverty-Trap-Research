...
w_g(1,:) = 0:delta_N:w_max;  % Uniform Distribution of wealth in first time period
...
for j=1:T
     h = histogram(w_g(j,:), edges);
     bins(j,:)= h.Values; 
end
%Plot bins matrix in bargraph

figure(1)
title('Wealth Polarization over Time, Ref-Dep Utility')
bar3(bins)
...
figure(2)
title('Wealth Polarization at Last Period, Ref-Dep Utility')
hold on;
