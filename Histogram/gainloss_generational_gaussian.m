...
%  a histogram plot.
%
%  Note, this call on functions b (bequeset function of wealth) 
...
T = 20; % periods
N = 1000; % number of people aka partitions for wealth distribution
%w_g=zeros(T,N);  % wealth of generations --- a matrix where at time period T, we obersve agent N's wealth
...
%w_g(1,:) = normrnd(.48,.125,[1,N]);  % Uniform Distribution of wealth in first time period
...
binNum = 20;   %bin number for 2d histogram
bins = zeros(T, binNum);  %creating empty bins
edges = [0:1/binNum:1];

for j=1:T
    h = histogram(w_g(j,:), edges);   %storing each period's histogram
    bins(j,:)= h.Values;               %Putting them all in bins matrix
end

%Plot bins matrix in bargraph
figure(1)
title('Wealth Polarization over Time, Gaussian Dist.')
bar3(bins)
set(gca,'XTickLabel', [0:2/binNum:1], 'XTick', [1:2:20])
xlabel('Wealth')
ylabel('Period')
...
figure(2)
title('Generational Wealth Polarization, Loss Averse', 'FontSize', 14)
hold on;
histogram(w_g(1,:),10, 'DisplayName','First Generation')
histogram(w_g(T,:),10, 'DisplayName','20th Generation')
xlabel('Wealth', 'FontSize', 16)
ylabel('Number of Families', 'FontSize', 16)
legend('FontSize', 14)
set(gcf, 'Position',  [300, 100, 1000, 666])   
set(gca, 'FontSize', 12)
